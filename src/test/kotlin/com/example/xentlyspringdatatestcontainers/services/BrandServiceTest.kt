...
class BrandServiceTest {
    ...
    inner class GetMultiple {
        ...
        fun `fallback to sql db result if search optimised db is empty`(query: String?) {
            ...
            Mockito.`when`(sqlDataSource.get(query, pageable))
                .thenReturn(Page.empty())
    ...
    inner class GetSingle {
        ...
        @Test
        fun `does not save sql db results to search optimised db if result is null`() {
            val slug = "example"
            val sqlDataSource = Mockito.mock(BrandDataSource::class.java)
            val searchDataSource = Mockito.mock(BrandDataSource::class.java)
            val service = BrandService(sqlDataSource, searchDataSource)
            ...
            assertThrows(BrandNotFoundException::class.java) {
                service.get(slug)
            }
